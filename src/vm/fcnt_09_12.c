...
/*   Updated: 2019/04/19 19:14:54 by prastoin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool		zjmp(t_vm *game, t_process *process, int32_t param[4], uint8_t ocp)
{
	...
	offset = (process->offset + (int32_t)param[0] - 1);
	while (offset < 0)
		offset += MEM_SIZE;
...
void		ft_dump_process(t_process *process)
{
	...
	for(size_t i = 0; i < REG_NUMBER; i++)
		printf("\tReg %zu: %d, %u-%u-%u-%u\n", i, conv_bin_num(process->registre[i], REG_SIZE), process->registre[i][0], process->registre[i][1], process->registre[i][1], process->registre[i][2], process->registre[i][3]);
}
...
bool		sti(t_vm *game, t_process *process, int32_t param[4], uint8_t ocp)
{
	...
	mem_write(game->mem, process->registre[param[0]], (process->offset - 2 + adress) % MEM_SIZE, REG_SIZE);
...
bool		ft_fork(t_vm *game, t_process *process, int32_t param[4], uint8_t ocp)
{
	...
	*new_process = (t_process) {
		.offset = (process->offset + (param[0] % IDX_MOD) - 1) % MEM_SIZE, // TODO: Negative offset
		.is_alive = true
	};
